    ...
    return (
        <>
            ...
            <Checkbox
                ...
                // size={size ?? 'sm'} // Removed size prop as customized Checkbox might not support it, defaulting to className control
...
function DataGridTableRowSelectAll({ size }: { size?: 'sm' | 'md' | 'lg' }) {
    ...
    return (
        <Checkbox
            ...
            // size={size} // Removed size prop for safety
            className="align-[inherit]"
        />
    );
}
...
function DataGridPagination(props: DataGridPaginationProps) {
    ...
    return (
        <div
            ...
            className={cn(
                'flex flex-wrap flex-col sm:flex-row justify-end items-center gap-2.5 py-2.5 sm:py-0 grow',
        ...
        <DataGridTableHeadRowCell header={header} dndStyle={style} dndRef={setNodeRef}>
            <div className="flex items-center justify-start gap-0.5">
                <Button
                    // mode="icon" // REMOVED
                    size="sm"
                    variant="secondary" // Changed from dim to secondary as dim might not exist
...
function DataGridTableDndRowHandle({ rowId }: { rowId: string }) {
    ...
    return (
        <Button variant="secondary" size="sm" className="size-7" {...attributes} {...listeners}>
            <GripHorizontal />
        </Button>
    );
}
