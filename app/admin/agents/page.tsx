...
import { columns, Agent } from './columns';
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (error) {
          ...
          setData(agents as any[] || []);
        }
      } catch (error) {
        console.error('Error in agents fetch:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="flex flex-col h-full space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">All Agents</h1>
      </div>
